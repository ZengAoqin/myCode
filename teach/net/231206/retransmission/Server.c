...
void* recvFun(void* arg)
{
	...
	for(;;)
	{
		...
		if(!strcmp(buf, "byebye\n") || !strcmp(buf, ""))
			break;
		strcpy(text, buf);
		sprintf(buf, "%s:%s", inet_ntoa(p->ClientAddr.sin_addr), text); //组合消息和ID
		//广播消息
		printf("当前人数:%d\n", count);
		for(int i = 0; i < count; i++)
		{
			write(clientInfo[i].confd, buf, strlen(buf));
		}
	}
...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		//将连接成功的客户端保存到结构体数组中
		clientInfo[count].confd = confd; //客户端的套接字
		memcpy(&clientInfo[count].ClientAddr, &ClientAddr, sizeof(ClientAddr));
		printf("[%s]客户端连接成功\n", inet_ntoa(ClientAddr.sin_addr));
		pthread_create(&thread, NULL, recvFun, clientInfo+count);
		count++; //记录下一个
	}
