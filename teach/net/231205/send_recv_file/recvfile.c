...
int main(int argc, char* argv[])
{
	...
	printf("send:%s\n", fbuf);
	////mnt/hgfs/share/teach/231205/send_recv_file/test/
	//设置保存文件的工作路径
	char sbuf[128] = {0};
	sprintf(sbuf, "/mnt/hgfs/share/teach/231205/send_recv_file/test/%s", fbuf);
	//创建文件操作
	int fd = open(sbuf, O_CREAT|O_RDWR, 0666);
	if(-1 == fd)
	{
