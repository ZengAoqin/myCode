...
bool double_list_circul_find(pDoubleList head, pDoubleList node)
{
	if(head == NULL || node == NULL)
	{
		printf("不能传空指针\n");
...
void double_list_circul_delete(pDoubleList head, pDoubleList node)
{
	if(node == NULL || head == NULL)
	{
		printf("节点指针不能为空\n");
	...
	if(!double_list_circul_find(head, node))
	{
		return;
	}
...
int get_double_list_circul_index(pDoubleList head, pDoubleList Node)
{
	if(head == NULL || Node == NULL)
	{
		printf("节点指针不能为空\n");
	...
	if(!double_list_circul_find(head, Node)) return -1;
...
void double_list_circul_insert(pDoubleList head, pDoubleList fristNode, pDoubleList secondNode, pDoubleList Node)
{
	if(head == NULL || fristNode == NULL || secondNode == NULL || Node == NULL)
	{
		printf("节点指针不能为空\n");
		return;
	}
	
	if(!double_list_circul_find(head, fristNode)) return;
	if(!double_list_circul_find(head, secondNode)) return;
	
	pDoubleList pFront = fristNode;
	pDoubleList pBack = secondNode;
	
	if(fristNode == secondNode && fristNode != head) return; //不能将头节点删除
	
	//secondNode不能在fristNode之前
	int nF = get_double_list_circul_index(head, fristNode);
	int nS = get_double_list_circul_index(head, secondNode);
	...
	pDoubleList pF = pFront->pNext;
	pDoubleList pS = pBack;
	
	while(pF != pS)
	{
		double_list_circul_delete(head, pF);
		pF = pFront->pNext;
	}
	
	double_list_insert_node_front(head, pS, Node);
}
...
int main(int argc, char* argv[])
{
	...
	pDoubleList node6 = create_node(108);
	double_list_circul_insert(head, node2, node3, node6); //Double head-108-Double head
	showDoubleListCircul(head);
